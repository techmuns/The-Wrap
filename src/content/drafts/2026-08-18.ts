import type { Issue } from "@/types/issue";

// AUTO-GENERATED DRAFT by scripts/build-weekly-issue.ts on 2026-08-18T08:56:22.969Z.
// This is a reviewable draft, NOT a published issue. Edit the "[DRAFT — write this]" fields
// (headline, dek, section commentary), then promote to src/content/issues/ to
// publish. See docs/WEEKLY-ISSUE.md.
const issue: Issue = {
  "slug": "2026-08-18",
  "date": "18 Aug 2026",
  "isoDate": "2026-08-18",
  "title": "[DRAFT — write this]: headline for the week of 18 Aug 2026",
  "dek": "[DRAFT — write this]: one-line standfirst.",
  "readingTime": "Draft preview",
  "sections": [
    {
      "id": "summary",
      "title": "The week in one line",
      "body": [
        "[DRAFT — write this]: the week's thesis — index moves, the macro backdrop, and the single takeaway.",
        "Over the past week (1 trading day captured): 14 insider/promoter buys and 21 sells, 4 block and 100 bulk deals, 49 notable filings, 25 earnings calls, and 125 corporate actions."
      ]
    },
    {
      "id": "breadth",
      "title": "Market breadth",
      "body": [
        "[DRAFT — write this]: is participation broadening or narrowing? (No live breadth feed yet — a Market Breadth tool is on the roadmap.)"
      ],
      "note": "A Market Breadth tool (sector × EMA heatmap) is planned under Data Tools."
    },
    {
      "id": "insider",
      "title": "Insider & promoter trades",
      "body": [
        "[DRAFT — write this]: one line on the week's promoter/insider tone — accumulation, distribution, or two-sided."
      ],
      "groups": [
        {
          "heading": "Notable buying",
          "items": [
            {
              "text": "Balu Forge — Promoter And Director (2) (Trimaan Jaspalsingh Chandock, Jaikaran Jaspalsingh Chandock), ₹108.00 cr",
              "starred": true
            },
            {
              "text": "Fischer Medical — Promoter (2) (Shankar Varadharajan, Fmv Holdings Pte Ltd), ₹31.21 cr"
            },
            {
              "text": "Ponni Sug.Erode — Seshasayee Paper And Boards Limited (Promoter), ₹10.15 cr"
            },
            {
              "text": "Dec.Gold Mines — Subramaniam Sundaram (Director), ₹4.46 cr"
            },
            {
              "text": "Samkrg Pistons — Saripalli Kishore (Promoter And Director), ₹2.30 cr"
            }
          ]
        },
        {
          "heading": "Notable selling",
          "items": [
            {
              "text": "Leap India — Vertical Holdings Ii Pte. Ltd (Promoter), ₹1,999.00 cr",
              "starred": true
            },
            {
              "text": "V-Guard Industri — Other (4) (Priya Sarah Cheeran Joseph, Priya Sarah Cheeran Jo...), ₹38.71 cr"
            },
            {
              "text": "RPP Infra Proj. — Ms. N A Yagavi (Promoter Group), ₹10.22 cr"
            },
            {
              "text": "HCL Technologies — Hcl Technologies Stock Options Trust (Designated Person), ₹2.76 cr"
            },
            {
              "text": "Jubilant Ingrev. — Trust (2) (Jubilant Ingrevia Employees Welfare Trust, Jubilan...), ₹1.94 cr"
            }
          ]
        }
      ],
      "note": "Ranked by disclosed value over the past week. The full list lives in the Buying & Selling tracker.",
      "link": {
        "href": "/data-tools/insider-trades",
        "label": "Buying & Selling"
      }
    },
    {
      "id": "deals",
      "title": "Bulk & block deals",
      "body": [
        "[DRAFT — write this]: a line on the week's institutional churn — who's building or trimming positions."
      ],
      "groups": [
        {
          "heading": "Block deals",
          "items": [
            {
              "text": "Baazar Style Retail Ltd — ADITYA KUMAR HALWASIYA bought ₹162.90 cr",
              "starred": true
            },
            {
              "text": "Baazar Style Retail Ltd — SHREYANS SURANA sold ₹76.02 cr"
            },
            {
              "text": "Baazar Style Retail Ltd — BHAGWAN  PRASAD sold ₹54.30 cr"
            },
            {
              "text": "Baazar Style Retail Ltd — SIDHARTH  SURANA sold ₹32.58 cr"
            }
          ]
        },
        {
          "heading": "Bulk deals",
          "items": [
            {
              "text": "Embassy Office Parks REIT — APAC COMPANY XXIII LIMITED sold ₹1,162.70 cr"
            },
            {
              "text": "Embassy Office Parks REIT — APAC COMPANY XXIII LIMITED sold ₹1,162.59 cr"
            },
            {
              "text": "Baazar Style Retail Ltd — ADITYA KUMAR HALWASIYA bought ₹162.90 cr"
            },
            {
              "text": "Dhoot Transmission Ltd — NK SECURITIES RESEARCH PRIVATE LIMITED sold ₹131.59 cr"
            }
          ]
        }
      ],
      "note": "Largest by deal value over the past week. Live bulk, block and short deals are tracked here.",
      "link": {
        "href": "/data-tools/bulk-block-deals",
        "label": "Bulk & Block Deals"
      }
    },
    {
      "id": "announcements",
      "title": "Noteworthy announcements",
      "body": [
        "Our pick of the week's more consequential filings, by category — not exhaustive:"
      ],
      "groups": [
        {
          "heading": "Capex & New Ventures",
          "items": [
            {
              "text": "KPI Green Energy — Energized 130 MW AC solar capacity; cumulatively 269.7 MW AC at Bharuch hybrid IPP on August 17, 2026.",
              "starred": true
            },
            {
              "text": "Indag Rubber — Q1 FY27 revenue rose 26% to ₹60.45 crore; EBITDA and PAT jumped, and subsidiary started commercial production."
            }
          ]
        },
        {
          "heading": "Order Wins",
          "items": [
            {
              "text": "Highway Infra — NHAI awarded Highway Infrastructure ₹80.17 crore Palayam Fee Plaza operation contract for 90 days.",
              "starred": true
            },
            {
              "text": "Sigma Advanced System — Sigma Advanced Systems wins ₹155 crore Home Ministry orders for Indrajaal Ranger on August 17, 2026."
            },
            {
              "text": "NIS Management — Q1 FY27 income ₹115.44 Cr, PAT up 35%; won ₹45.71 Cr Reliance orders."
            }
          ]
        },
        {
          "heading": "Acquisitions",
          "items": [
            {
              "text": "Lloyds Engineeri — Lloyds Engineering acquired 51.13% of SISCOL for Rs 626.4 crore; SISCOL became subsidiary on Aug 17, 2026.",
              "starred": true
            },
            {
              "text": "Lloyds Engineeri — Lloyds Engineering acquired 51.13% of SISCOL for Rs 626.4 crore; transaction consummated on August 17, 2026."
            },
            {
              "text": "Manipal Health — Manipal Health signed BTA on Aug 17, 2026 to acquire Kinder Women’s Hospital operations in Bengaluru for ₹130 crore."
            }
          ]
        },
        {
          "heading": "Joint Ventures",
          "items": [
            {
              "text": "True Colors — Board approved JV with identified partners for paper/specialty paper business; appointed internal, cost and secretarial auditors."
            }
          ]
        },
        {
          "heading": "Open Offers",
          "items": [
            {
              "text": "ACI Infocom — Credora Partners open offer for up to 3.70 crore shares at ₹1.53 each, dated August 17, 2026."
            }
          ]
        },
        {
          "heading": "Fund Raising",
          "items": [
            {
              "text": "ACI Infocom — EGM on Sept 9, 2026 to alter objects, raise capital, and issue shares/warrants worth about Rs.50 crore."
            },
            {
              "text": "Lloyds Engineeri — Allotted 7,07,42,458 shares at ₹71.25 on Aug 17, including SISCOL share-swap acquisition and ₹4.99 crore cash allotment."
            },
            {
              "text": "Gamco Ltd — Board approved preferential issue of 43.06 lakh shares at Rs50 each, raising Rs21.53 crore; AGM on 16 Sep 2026."
            }
          ]
        },
        {
          "heading": "Rights Issue",
          "items": [
            {
              "text": "Key Corp — Board meeting on 20 August 2026 to consider rights issue of equity shares up to 15 crore."
            },
            {
              "text": "Atal Realtech — Board approved rights issue fund raising of up to Rs.16 crore on August 17, 2026."
            },
            {
              "text": "Bonlon Industrie — Committee meets 20 Aug 2026 to decide rights issue price and terms for ₹49.75 crore issue."
            }
          ]
        }
      ],
      "note": "Every category here is a filtered view over the unified announcements feed.",
      "link": {
        "href": "/data-tools/announcements",
        "label": "Announcements"
      }
    },
    {
      "id": "concalls",
      "title": "Earnings calls",
      "body": [
        "[DRAFT — write this]: which calls are worth your time and why."
      ],
      "groups": [
        {
          "heading": "Recent calls with materials",
          "items": [
            {
              "text": "Max Healthcare — View Presentation"
            },
            {
              "text": "ION Exchange — View Transcript, View Summary"
            },
            {
              "text": "Addictive Learn — View Recording, View Summary"
            },
            {
              "text": "Colgate-Palmoliv — View Recording, View Summary"
            },
            {
              "text": "Chandan Healthca — View Presentation"
            },
            {
              "text": "Modi Naturals — View Presentation"
            }
          ]
        }
      ],
      "note": "Transcripts, notes and recordings are collected here.",
      "link": {
        "href": "/data-tools/concalls",
        "label": "Concalls"
      }
    },
    {
      "id": "corporate-actions",
      "title": "Corporate actions",
      "body": [
        "[DRAFT — write this]: any buyback or bonus worth flagging."
      ],
      "groups": [
        {
          "heading": "Buyback",
          "items": [
            {
              "text": "Indl. Inv. Trust — Tender Offer · 150.00 · 25.00"
            },
            {
              "text": "Orbit Exports — Tender Offer · 250.00 · 27.60"
            },
            {
              "text": "Team Lease Serv. — Tender Offer · 1600.00 · 238.00"
            },
            {
              "text": "Rolex Rings — Tender Offer · 180.00 · 180.00"
            }
          ]
        },
        {
          "heading": "Bonus",
          "items": [
            {
              "text": "Mayank Cattle Fo — Ratio 1:1"
            },
            {
              "text": "Goodluck India — Ratio 2:1"
            },
            {
              "text": "Kahan Packaging — Ratio 3:1"
            },
            {
              "text": "Organic Recyclin — Ratio 1:2"
            }
          ]
        },
        {
          "heading": "Stock Split",
          "items": [
            {
              "text": "Spice IslandsInd — 10.00 · 2.00"
            },
            {
              "text": "DSP Gold ETF — 10.00 · 1.00"
            },
            {
              "text": "DSP Silver ETF — 10.00 · 1.00"
            },
            {
              "text": "Waterways Leisur — 10.00 · 1.00"
            }
          ]
        },
        {
          "heading": "Rights Issue",
          "items": [
            {
              "text": "Containe Tech. — Ratio 2:1 · ₹5.00 premium"
            },
            {
              "text": "Alan Scott Ente. — Ratio 1:6 · ₹65.00 premium"
            },
            {
              "text": "Viceroy Hotels — Ratio 6:7 · ₹105.00 premium"
            },
            {
              "text": "Kshitij Polyline — Ratio 2:5 · ₹1.17 premium"
            }
          ]
        },
        {
          "heading": "Dividend",
          "items": [
            {
              "text": "Adtech Systems — Final · 11.00%"
            },
            {
              "text": "Satia Industries — Final · 40.00%"
            },
            {
              "text": "Rithwik Facility — Final · 10.00%"
            },
            {
              "text": "Talbros Engg. — Final · 30.00%"
            }
          ]
        }
      ],
      "note": "Bonuses, buybacks, splits, rights and dividends — full list in the tracker.",
      "link": {
        "href": "/data-tools/corporate-actions",
        "label": "Corporate Actions"
      }
    },
    {
      "id": "curated",
      "title": "Curated",
      "body": [
        "[DRAFT — write this]: two or three long-reads worth the weekend."
      ],
      "link": {
        "href": "/curated",
        "label": "Curated"
      }
    }
  ]
};

export default issue;
